# MICROPLANIFICACIÓN POR LECCIÓN — VERSIÓN OPTIMIZADA

## Datos de identificación

- Grado: sexto de primaria
- Unidad: 05
- Lección: 01
- Parte: 01
- Área: Ciencia y Tecnología
- Tema: La Tierra, un planeta con condiciones para la vida
- Duración: 45 minutos
- Recurso central: juego HTML autocontenido `Expedición Hábitat`
- Organización: un solo equipo; el docente opera el mouse y el grupo toma las decisiones

## Resumen

La clase presenta a la Tierra como un planeta que reúne condiciones relacionadas que favorecen la vida conocida: agua líquida, una atmósfera, energía y un intervalo adecuado de temperatura. En `Expedición Hábitat`, el aula funciona como una sola tripulación científica. Antes de recibir toda la evidencia, el grupo decide si intentaría vivir en cada planeta; después analiza cuatro lecturas de sensores, emite un diagnóstico y observa una simulación que indica si la colonia podría sobrevivir, no sobreviviría o enfrentaría un riesgo que la evidencia aún no permite resolver. Al final compara los cuatro mundos y selecciona el más habitable.

## Objetivos

### Objetivo de la clase

Explicar cómo el agua líquida, la atmósfera, la energía y una temperatura adecuada se relacionan para permitir la vida, y usar evidencia para elegir el planeta que reúne condiciones más favorables.

### Objetivos específicos

- Identificar evidencias relacionadas con agua, atmósfera, energía y temperatura.
- Clasificar cada condición como favorable, crítica o incierta.
- Integrar las cuatro condiciones en un diagnóstico: favorable, no favorable o evidencia insuficiente.
- Explicar que una condición puede afectar a otra; por ejemplo, una atmósfera puede contribuir a regular la temperatura.
- Diferenciar entre un planeta con condiciones favorables y un planeta donde la vida ya está comprobada.
- Revisar una decisión inicial cuando nueva evidencia la contradice.

## Desarrollo

### Antes de ingresar al aula

1. Abre `6P/lección 1/index.html` en el navegador. No requiere conexión a internet.
2. Conecta la computadora al televisor o proyector y pulsa `Pantalla completa`.
3. Comprueba el sonido. Puede desactivarse desde el botón superior sin afectar la actividad.
4. Ubica a los estudiantes de modo que todos observen la pantalla. El docente será el único que manipule el mouse.
5. Explica que no habrá puntajes ni competencia: el producto es un diagnóstico colectivo basado en evidencia.

Plan alternativo: si falla la pantalla, lee los cuatro expedientes de esta planificación y registra en la pizarra la decisión previa, las cuatro clasificaciones, el diagnóstico y la revisión final.

### 1. Apertura: una decisión antes de conocer todo — 0 a 6 minutos

Proyecta la portada `¿Dónde podríamos vivir?` y lee el objetivo de la misión. Pregunta:

- ¿Qué necesita un ser vivo para mantenerse con vida?
- ¿Bastaría con encontrar agua en cualquier estado?
- ¿Un planeta bonito necesariamente sería habitable?

Pulsa `Iniciar expedición`. Presenta la regla central: primero el grupo asumirá el riesgo de decidir y después recibirá la evidencia. La decisión puede cambiar cuando aparecen datos nuevos; cambiarla no es un error, sino una práctica científica.

Evidencia esperada: el grupo anticipa al menos dos condiciones y reconoce que una imagen no prueba habitabilidad.

### 2. Construcción del concepto: las condiciones forman un sistema — 6 a 12 minutos

Antes del primer expediente, construye en la pizarra una red simple:

- Agua líquida: medio donde ocurren procesos vitales conocidos.
- Atmósfera: aporta gases, protección y regulación térmica.
- Energía: hace posibles los procesos, pero su exceso también puede ser perjudicial.
- Temperatura adecuada: permite que el agua permanezca líquida durante suficiente tiempo.

Une las condiciones con flechas. Señala dos relaciones: la atmósfera influye en la temperatura; la energía recibida influye en la temperatura y en el estado del agua. Concluye que ninguna condición debe evaluarse de forma aislada.

### 3. Expedición cooperativa: cuatro mundos — 12 a 36 minutos

El juego contiene cuatro planetas. En todos se repite el mismo ciclo, lo que permite comparar sin cambiar el procedimiento.

#### Ciclo de investigación por planeta

1. Observación inicial: la imagen ocupa toda la pantalla y el panel flotante muestra una breve descripción.
2. Decisión previa: el grupo elige `Aterrizaríamos` o `Seguir buscando`.
3. Escaneo: aparecen, una por una, las lecturas de agua, atmósfera, energía y temperatura.
4. Clasificación: el grupo decide si cada lectura es `Favorable`, `Crítica` o `No sabemos`.
5. Diagnóstico integral: el grupo selecciona `Favorable para vivir`, `No favorable` o `Falta evidencia`.
6. Simulación: la pantalla confronta la decisión previa con las condiciones reales y anuncia la consecuencia.
7. Revisión: se observan las relaciones causales que explican el resultado y se continúa al siguiente mundo.

El docente conduce el diálogo y registra con el mouse la decisión acordada. No hay casillas de confirmación docente ni comprobaciones externas al razonamiento visible en el juego.

#### Mundo 1: Nereida

- Agua: grandes superficies de agua líquida.
- Atmósfera: capa gaseosa estable que filtra radiación.
- Energía: luz constante sin exceso.
- Temperatura: valores moderados en gran parte de la superficie.
- Diagnóstico esperado: favorable.
- Simulación al aterrizar: la colonia podría sobrevivir.
- Precisión científica: las condiciones favorables no demuestran que ya exista vida.

#### Mundo 2: Brasa

- Agua: no hay agua líquida estable por el calor.
- Atmósfera: muy densa; atrapa demasiado calor.
- Energía: recibe una cantidad extrema.
- Temperatura: incompatible con la supervivencia de una colonia.
- Diagnóstico esperado: no favorable.
- Simulación al aterrizar: la colonia no sobreviviría.
- Relación clave: energía excesiva y atmósfera densa agravan juntas la temperatura.

#### Mundo 3: Umbral

- Agua: rastros de hielo, sin agua líquida estable confirmada.
- Atmósfera: casi ausente.
- Energía: disponible, pero concentrada en el lado iluminado.
- Temperatura: extrema entre el día y la noche.
- Diagnóstico esperado: no favorable.
- Simulación al aterrizar: la colonia no sobreviviría.
- Relación clave: la energía por sí sola no basta; sin atmósfera no se distribuye el calor.

#### Mundo 4: Cripta

- Agua: posible océano bajo el hielo, todavía no confirmado.
- Atmósfera: muy delgada.
- Energía: escasa desde la estrella; existe una fuente interna posible.
- Temperatura: superficie permanentemente congelada.
- Diagnóstico esperado: evidencia insuficiente.
- Simulación al aterrizar: el riesgo es demasiado alto.
- Relación clave: una señal prometedora no permite ignorar la falta de evidencia sobre las demás condiciones.

### 4. Consejo final: elegir con evidencia — 36 a 42 minutos

La pantalla reúne los cuatro diagnósticos. Pide al grupo comparar mundos y elegir dónde establecería la colonia. Antes de pulsar `Comprobar decisión final`, solicita que el grupo identifique qué combinación de condiciones respalda su elección.

Resultado esperado: Nereida. Si el grupo elige otro planeta, la simulación explica qué combinación impide la supervivencia y permite cambiar la elección. La meta no es acertar a la primera, sino revisar el diagnóstico hasta que la decisión sea coherente con la evidencia.

### 5. Cierre y evidencia de aprendizaje — 42 a 45 minutos

Recupera el objetivo mostrado al inicio y completa colectivamente:

> Elegimos Nereida porque reúne agua líquida, una atmósfera protectora, energía suficiente y temperatura moderada. Estas condiciones se relacionan y hacen posible la vida conocida, pero no prueban que allí ya exista vida.

Realiza tres preguntas de salida:

- ¿Por qué una atmósfera puede modificar la temperatura?
- ¿Por qué encontrar hielo no equivale a encontrar agua líquida disponible?
- ¿Qué diferencia hay entre “podría permitir la vida” y “tiene vida”?

## Evaluación

La evidencia es el diagnóstico colectivo registrado en el juego y la explicación final. Se considera logrado cuando el grupo:

- clasifica las cuatro condiciones usando la evidencia mostrada;
- integra las condiciones en un diagnóstico coherente;
- explica al menos una relación causal entre condiciones;
- elige Nereida por la combinación de condiciones y no por su apariencia;
- mantiene cautela científica sobre la existencia de vida.

## Ficha de aplicación

- Archivo principal: `6P/lección 1/index.html`
- Tecnología: HTML, CSS y JavaScript puros; sin backend.
- Conectividad: opcional; funcionamiento completo sin internet.
- Control: un mouse manejado por el docente.
- Modalidad: equipo único, decisión cooperativa.
- Estudiantes: funciona con cuatro, seis o cualquier número que pueda deliberar como grupo.
- Recursos visuales: cuatro imágenes planetarias originales generadas para esta lección e incrustadas en el HTML.
- Accesibilidad: controles grandes, contraste alto, foco visible, opción de silenciar y respeto de movimiento reducido.
